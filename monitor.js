...
function watchInbox() {
  ...
  function extractTableDataFromText(textBody) {
    ...
    // Process each line in the extracted table
    lines.forEach((line, index) => {
      console.log('Processing line:', line);
  
      // Use regex to capture the columns accurately, accounting for possible extra spaces
      const columns = line.match(/(\S+)\s+([A-Za-z-]+)\s+(Line-\d+)\s+([A-Za-z\s\(\)+]+)\s+([\d,.]+)\s+([\d,.]+)\s+([\d,.]+)\s+([\d,.]+)\s+([\d,.]+)/);
      ...
      if (columns && columns.length === 10) {
        writer.write({
          ...
          'Line': columns[3],
          'Category': columns[4],
          'A-Shift': columns[5].replace(',', ''),
          'B-Shift': columns[6].replace(',', ''),
          'C-Shift': columns[7].replace(',', ''),
          'A+B+C Shift': columns[8].replace(',', ''),
          '% - Percentage': columns[9].replace(',', '')
        });
      } else {
